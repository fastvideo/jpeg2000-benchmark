// nvj2k_bench version 02, 31 August 2026 - what changed is listed below
// ---------------------------------------------------------------------------
// nvJPEG2000 benchmark harness
//
// THE VERSION IS IN THREE PLACES AT ONCE, ON PURPOSE:
//   - in the file name          nvj2k_bench-02.cpp, visible in the folder
//   - on the first line above   visible the moment the file is opened
//   - in the built program      nvj2kDecoderSample -version prints it and
//                               stops, measuring nothing
// An older executable does not know -version and prints its usage instead,
// which is answer enough. bench-05.py asks both executables for their version
// before the first measurement and stops the run if it does not match this
// file. That is what makes "which build produced these numbers" a question
// with an answer rather than a guess.
//
// WHAT CHANGED SINCE VERSION 01 (the file called nvj2k_bench.cpp)
//   1. The -version option above.
//   2. The threads-and-batch encoder used to print "including all transfers"
//      even when it was started with -noupload and the upload was outside the
//      measured region. The label is now chosen by the same flag that decides
//      what is measured, exactly as the decoder already does. A label that
//      lives apart from the code that makes it true is not evidence.
//   3. The usage line lists -nodownload and -version, which it did not.
// Nothing that affects a measurement was changed: the timers, the boundaries
// and the defaults are those of version 01.
//
...
static Args parseArgs(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        // Answered before the device is touched, so that asking a program
        // which build it is costs nothing and measures nothing.
        else if (eq(k, "-version")) {
            std::printf("nvj2k_bench version 02, 31 August 2026\n");
            std::exit(0);
        }
...
static void benchEncode(const Args& a, const Image& im) {
    ...
    // The label is chosen by the same flag that decides what was measured:
    // with -noupload the pixels are put on the card before the timer starts,
    // and saying "including all transfers" would then be untrue.
    std::printf("- GPU pipeline %s for %d images "
                "per %d thread%s = %.1f ms; %.1f FPS;\n",
                a.noupload ? "excluding the host-to-device transfer"
...
int main(int argc, char** argv) {
    ...
    if (a.input.empty()) {
        ...
        std::printf("usage: %s -i input -o output [-a irrev|rev] [-c 32] "
                    ...
                    "[-noupload] [-nodownload] [-version]\n",
                    argv[0]);
        std::printf("  -noupload    the host-to-device copy is done before "
                    "the timer starts\n");
        std::printf("  -nodownload  the decoded frame is not copied back to "
                    "host memory;\n"
                    "               this is what mirrors the Fastvideo sample "
                    "in single frame mode\n");
        std::printf("  -version     print the version of this harness and "
                    "stop\n");
        return 1;
    }
